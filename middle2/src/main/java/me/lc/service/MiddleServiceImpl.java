...
public class MiddleServiceImpl implements MiddleService {
    ...
    public MiddleProcessorResponseBean getBadTraces() {
        if (MiddleProcessor.READ_FINISH) {
            ...
            LOGGER.info("Middle 2 read finish response bad traces size = "+responseResult.size());
            responseBean.setBadTraces(responseResult);
            return responseBean;
        } else {
            if (prevGetBatchPos >= MiddleProcessor.currProcessingBadPos) {
                LOGGER.info("Middle 2 response bad traces null");
                return null;
            } else {
                ...
                responseBean.setBadTraces(responseResult);
                LOGGER.info("Middle 2 response bad traces size = "+responseResult.size());
                return responseBean;
            }
        }
    }
    ...
    public Map<String, List<String>> getRelatedTraces(Set<String> traceIds) {
        ...
        if (MiddleProcessor.READ_FINISH) {
            ...
            LOGGER.info("middle 2 read finish response related traces, size="+responseResult.size());
...
//            }
            LOGGER.info("middle 2 response related traces, size="+responseResult.size());
            return responseResult;
        }
    }
}
